...
Evaluation rules:
    ...
    eval({call-static E-op E1 E2},env)
                                = eval(Ef,extend(x2,eval(E2,env),extend(x1,eval(E1,env),envf)))   ;; We need to add both symbols in the function environment
  ...
  (cases expr
    ...
    [(If cond true-cond false-cond)
     (let ([cval (eval cond env)])
       (cases cval
         [(BoolV false) (eval false-cond env)]
...
(define (createGlobalEnv)
  (Extend 'second <-- fill in -->
          (Extend 'first 
                  (Extend 'cons (FunV 'f 's (Union (Id 'f) (Id 's)) (EmptyEnv)) ;; fix
                          (EmptyEnv)))))
...
(define (run str)
  (let ([result (eval (parse str) (EmptyEnv))])
    (cases result
      [(SetV S) S]
      [(BoolV b) b]
      [(FunV s1 s2 body e) (FunV s1 s2 body e)])))
      ;[else (error 'run "evaluation returned invalid value: ~s" result)])))


(run "{with {cons {fun {f s} {union f s}} c{}} cons}") ;; fix
(run "{with {first {fun {p spare-param} {call-< -- fill in -- > }} first}")
