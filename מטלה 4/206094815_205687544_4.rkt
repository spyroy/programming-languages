...
for the 'cons' case we use the 'selector' function (just like the selector in class) here we call it op,
...
the last lecture realy helped us to figured this out and we transleted the code from the lecture
as below, and we also used the help of a friend from Ben Gurion University, this question was pretty tough for us.
...
#|------how we used the code from the last lecture------
(define (mycons x y)
  (define (pair selector)
    (selector x y))
  pair)
    |
define (mycons) >> fun {f s}
  define (pair selector) >> fun {op d}
    (selector x y) >> {call-dynamic op f s}
  pair >> cons

(define (myfirst p)
  (p (lambda (a b) a)))
        |
define (myfirst p) >> fun {p spare-param}
  (p (lambda (a b) a)) >> {call-static p {fun {a b} a}

(define (mysecond p)
  (p (lambda (a b) b)))
        |
define (mysecond p) >> fun {p spare-param}
  (p (lambda (a b) b)) >> {call-static p {fun {a b} b}

when we define we make function therfore, define >> fun.
when we apply afunction then we call it therefore, (some-fun arg1 arg2) >> (call-static/dynamic arg1 arg2).
when we call a function with a lambda we acctually have two functions because lambda is
also a function therefore, (some-fun (lambda (arg1 arg2) do-somthing)) >> (call-static/dynamic some-fun (fun (arg1 arg2) do-somthing)).
|# 
