...
for example - '((1 (2 3 5) 4)) will return 1 instead of 2.
|#
...
(test (count-3listsRec '((1 (2 3 5) 4))) => 1)
...
then asigned S - to the symbol, STR - to the string and K - to the KeyStack.
...
if it found nothing (or got EmptyKS) than it returns #f (aka false)
...
;; the function recives a boolean function and a list where all elements in the list with the same type (A) and returns Boolean.
;; the function returns true if and only if all elements in the list returned true on the given boolean function.
;; this function is recursively checking all elemnts in the list if the list is empty it will return true trivially
; otherwise all other elements have to return true from the given boolean function (the and condition).
...
;;(elements in the first list has to be from the same type (A) and all elements in the second list has to be from the same type (B)) and returns boolean.
...
(test (not(even-odd? (list 2 4 6) (list 1 2 5))))
